...
%               if hist_mode~=0, then function plots freqs of each observ on 
%               figure(10*i_obs+2)
...
%   frequencies, M=size(signal,2) is number of observations  
...
if filename~=0
    for i_obs=1:N_obs
    xlswrite(filename, freqs(:,:,i_obs),i_obs); % saves each observ. in a sheet
    end
end
