...
for k=1:(length(flim)-1) % k stands for component to be estimated
    ...
    F_s = choose_freq(freqs(:,:,1),segSNR(:,1),flim(k),flim(k+1));
...
%     subplot(2,1,2); plot([df_est,df_mean])
    plot([f_est,f_mean,ff0*1000*ones(length(f_est),1), (ff0*1000+200)*ones(length(f_est),1), (ff0*1000-200)*ones(length(f_est),1)])
